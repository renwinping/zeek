...
Plugin::hook_list Plugin::EnabledHooks() const
	{
	return zeek::plugin::plugin_mgr->HooksEnabledForPlugin(this);
	}

void Plugin::EnableHook(zeek::plugin::HookType hook, int priority)
	{
	zeek::plugin::plugin_mgr->EnableHook(hook, this, priority);
	}

void Plugin::DisableHook(zeek::plugin::HookType hook)
	{
	zeek::plugin::plugin_mgr->DisableHook(hook, this);
	}

void Plugin::RequestEvent(EventHandlerPtr handler)
	{
	zeek::plugin::plugin_mgr->RequestEvent(handler, this);
	}

void Plugin::RequestBroObjDtor(Obj* obj)
	{
	zeek::plugin::plugin_mgr->RequestBroObjDtor(obj, this);
	}

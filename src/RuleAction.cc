...
RuleActionAnalyzer::RuleActionAnalyzer(const char* arg_analyzer)
	...
	analyzer = zeek::analyzer::analyzer_mgr->GetComponentTag(arg.c_str());
	...
	if ( pos != string::npos )
		...
		child_analyzer = zeek::analyzer::analyzer_mgr->GetComponentTag(arg.c_str());
...
void RuleActionAnalyzer::PrintDebug()
	{
	if ( ! child_analyzer )
		fprintf(stderr, "|%s|\n", zeek::analyzer::analyzer_mgr->GetComponentName(analyzer).c_str());
	else
		fprintf(stderr, "|%s:%s|\n",
		        zeek::analyzer::analyzer_mgr->GetComponentName(analyzer).c_str(),
		        zeek::analyzer::analyzer_mgr->GetComponentName(child_analyzer).c_str());
	}
...
void RuleActionEnable::DoAction(const Rule* parent, RuleEndpointState* state,
	...
	if ( ! ChildAnalyzer() )
		{
		if ( ! zeek::analyzer::analyzer_mgr->IsEnabled(Analyzer()) )
			return;
		...
		{
		if ( ! zeek::analyzer::analyzer_mgr->IsEnabled(ChildAnalyzer()) )
			return;

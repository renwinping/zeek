...
void Manager::StartHeartbeatTimer()
	...
	zeek::timer_mgr->Add(new HeartbeatTimer(network_time + zeek::BifConst::Threading::heartbeat_interval));
	}
...
//	fprintf(stderr, "P %.6f %.6f do_beat=%d did_process=%d next_next=%.6f\n", network_time, zeek::timer_mgr->Time(), do_beat, (int)did_process, next_beat);
	}

...
class OuterIDBindingFinder : public zeek::TraversalCallback {
public:
	...
	zeek::TraversalCode PreExpr(const zeek::detail::Expr*) override;
	zeek::TraversalCode PostExpr(const zeek::detail::Expr*) override;

	std::vector<zeek::detail::Scope*> scopes;
	std::vector<const zeek::detail::NameExpr*> outer_id_references;
};

zeek::TraversalCode OuterIDBindingFinder::PreExpr(const zeek::detail::Expr* expr)
	{
...
zeek::TraversalCode OuterIDBindingFinder::PostExpr(const zeek::detail::Expr* expr)
	{
	if ( expr->Tag() == zeek::detail::EXPR_LAMBDA )
		scopes.pop_back();

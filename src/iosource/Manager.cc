...
PktSrc* Manager::OpenPktSrc(const std::string& path, bool is_live)
	...
	std::list<PktSrcComponent*> all_components = zeek::plugin::plugin_mgr->Components<PktSrcComponent>();
...
PktDumper* Manager::OpenPktDumper(const std::string& path, bool append)
	...
	std::list<PktDumperComponent*> all_components = zeek::plugin::plugin_mgr->Components<PktDumperComponent>();
	for ( const auto& c : all_components )
		{
		if ( c->HandlesPrefix(prefix) )
			{
